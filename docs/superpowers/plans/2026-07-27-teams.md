...
Expected: PASS. If `shared/state.test.ts` constructs a `MatchSettings` literal, add `teamCount: 0` to it.
...
```bash
git add shared/state.ts shared/gamemodes.ts shared/reduce.ts shared/gamemodes.test.ts shared/reduce.test.ts
...
- [ ] **Step 6: Fix the `Player` literal in `shared/standings.test.ts`**
...
Expected: PASS. Any other `Player` or `Entry` object literal the compiler flags gets `teamId: null` / `by: "<the owning player id>"` respectively.
...
```bash
git add shared/state.ts shared/teams.ts shared/teams.test.ts shared/reduce.ts shared/standings.test.ts
...
describe("rosterOf", () => {
  test("with teams off, one scorer per player", () => {
    const room = { ...withTeams(0, [null, null]), teams: [] };
...
describe("scoreRound with teams", () => {
  ...
  test("a one-member scorer scores exactly as a solo player always did", () => {
    const room = teamRoom(0, [null, null]);
    const results = scoreRound({
      scorers: rosterOf({ ...room, teams: [] }),
      entries: {
...
Then update the **existing** `scoreRound` tests in `shared/scoring.test.ts`: every `scoreRound({ players: [...], entries })` call becomes `scoreRound({ scorers: [...], entries })` with `Scorer` objects (`{ id, name, emoji, colorIndex: null, members: [id] }`), every `Entry` literal gains `by`, and every `results.players` becomes `results.scorers`. Every `alsoBy` assertion changes from emoji to scorer ids.
...
Expected: PASS. `shared/standings.test.ts` will fail on its `results()` helper — fix it now by renaming `players:` to `scorers:` in that helper and adding `colorIndex: null, members: [id]` to each row.
...
Then update the existing `computeStandings` tests in that file: they pass `Player[]`, which no longer typechecks. Convert each `player(id)` call at a `computeStandings` call site into `scorer(id, [id], null)`. The `player()` helper can be deleted once no test uses it.
...
export function HostTeams({ room, countdown }: Props) {
  ...
  return (
    <main className="screen screen--host host-teams">
      <HostHeader
        left={<h1 className="host-teams__title">Pick your team</h1>}
        round={1}
...
/* Every accented surface reads one custom property, set inline from
   TEAM_COLORS[colorIndex].token. Nothing here knows a colour name. */
.team-panel,
.team-tile,
.player-teams__mine {
