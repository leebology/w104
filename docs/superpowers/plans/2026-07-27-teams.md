...
**Run the tasks in this order, which is not their numbering:**

```
1 → 2 → 3 → 4 → 5 → 6 → 13 → 7 → 8 → 9 → 10 → 11 → 12 → 14 → 15
```

Task 4 introduces the `teams` phase, which makes tsc flag the unhandled case in `HostView` and `PlayerView` — the repo's only guard against a missing phase, so it must not be silenced with a `default:` branch. Task 13 adds those cases. Running 13 straight after 6 closes that window at three tasks instead of nine, and every task from 7 onward has a fully green build.

Task 13 is why `shared/protocol.ts` is edited in Task 5 rather than in Task 10: the team screens send those messages, so the wire types have to exist first. Numbering is otherwise untouched — each task is self-contained and its **Interfaces / Consumes** block is satisfied at the point this order dispatches it.
...
- Produces: `RoomEvent` gains `{ t: "joinTeam"; playerId; teamId: TeamId; now }`, `{ t: "leaveTeam"; playerId; now }`, `{ t: "setTeamName"; playerId; teamId: TeamId; name: string; now }`. `ClientMessage` gains the matching three cases.
...
- [ ] **Step 5: Add the matching wire messages to `shared/protocol.ts`**

The three events need client messages behind them before the team screens in Task 13 can send anything. Add the type-only import:
...
The Durable Object does not route them yet — that is Task 10. An unrouted case in `onMessage`'s `switch` is silently ignored, which is the same thing that happens to any unknown message today, so nothing breaks in between.
...
```bash
git add shared/reduce.ts shared/protocol.ts shared/reduce.test.ts
...
**Task 4 already did half of this** — it had to, because one of its own tests calls `startGame` from `teams`. The `from` set already includes `"teams"` and the destination already maps `"lobby" | "teams"` to `"voting"`. Do **not** assume the case is therefore finished: the line below that returns `enterTeams(room)` is still missing, and without it `startGame` from a lobby with teams on opens a countdown to voting having never built the teams — the room reaches voting with `teams: []` and nobody assigned.

Replace the whole body of `case "startGame"` with this version, which is a superset of what is there now:
...
**Interfaces:**
- Consumes: the three events from Task 5 and their `ClientMessage` cases, already added there; `MAX_TEAM_NAME_LEN`, `rosterOf` from Tasks 1 and 7.
- Produces: no new exports. The Durable Object now routes the team messages and fans shared lists out.
...
- [ ] **Step 1: Add the `load()` fallbacks in `party/server.ts`**
...
- [ ] **Step 2: Route the three messages in `onMessage`**
...
- [ ] **Step 3: Fan the accepted entry out to the team**
...
- [ ] **Step 4: Send the merged list on connect too**
...
- [ ] **Step 2: Confirm the optimistic entry already carries its author — do not redo this**

**Task 3 already did this** and its approach is the one to keep: `RoomStore` holds a `private playerId = ""` set from `opts.playerId` in `connect()`, and `submit` stamps `by: this.playerId`. That is better than reaching for `getPlayerId()` — it uses the id this socket actually connected with rather than re-reading local storage.

Verify `submit` still reads:
...
and leave it alone. Step 1 above replaces the `entryAck` accepted branch that Task 3 touched, which is the only overlap.
...
Fix the now-stale line in the "State flow" section. It currently reads that `startGame` "is legal from lobby, voting, and standings"; team select is a fourth:

```markdown
  `startGame` is legal from lobby, team select, voting, and standings, and is
  still the one exception (host solo-start bypasses `MIN_PLAYERS`), and
  `reduce` skips `settle` for it. From a lobby with teams on it opens team
  select rather than a countdown.
```
