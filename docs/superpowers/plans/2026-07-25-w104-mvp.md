...
> **This plan has been fully executed.** Several of its code blocks and stated numbers no longer match what shipped — see "Deviations discovered during implementation" below before trusting any number or snippet in a task.
...
This plan is a historical record, kept as originally written — its task code blocks are what was *planned*, not necessarily what shipped, and are not rewritten below. Read the current source, not these snippets, before trusting behaviour. What actually differs, and why:

1. **Task 3's `IDLE_REAP_MS` ships as 1 hour, not the `4 * 60 * 60 * 1_000` (4 hours) shown in Steps 4 and 10.** Shortened so a finished room releases its code sooner. The reap stays time-based rather than firing when the last socket closes, because a wifi drop or a locked phone must not destroy an in-progress round.
2. **Task 5's server implements `onAlarm()`, not an override of `alarm()`.** partyserver 0.5.8's own `alarm()` is `await this.#ensureInitialized(); await this.onAlarm();` — overriding `alarm()` (as Step 2's code block does) skips that initialization, so on a cold wake `onStart()` never runs, `this.room` is null, and the round hangs forever.
3. **Task 6's `src/net/identity.ts` falls back to a `Math.random`-based UUID v4** when `crypto.randomUUID` is unavailable, rather than calling it unconditionally as Step 1's code block does. `crypto.randomUUID` exists only in secure contexts, and the spec mandates plain http for LAN phone testing.
4. **The stated test totals were wrong; real counts govern.** Task 3 Step 11 says 41 (actually 51 once written), Task 4 Step 4 says 47 (actually 57), and the Self-review's "47 automated tests" is stale too. The suite finishes the plan at **65** tests. Tests were never deleted to hit a number.
5. **Task 4's "codes are uppercase for display" test as originally written is broken:** it draws two independent random codes and compares them, so it passes or fails independent of casing (~98.8% pass rate on chance alone) and asserts nothing about uppercasing. It was replaced with a single-draw version (`const code = makeRoomCode(); expect(code).toBe(code.toUpperCase())`). The clamp test was also strengthened with a `rand: () => 1` case alongside `0.999999`, because `0.999999` alone never actually reaches the clamp branch.
6. **Task 10's rationale for deleting the `default` branch was false.** Deleting a `switch` branch does not make TypeScript flag an unhandled phase on its own — with no explicit return type and no `noImplicitReturns`, a missing case just silently returns `undefined`. What actually makes the check real is an explicit `ReactElement` return type on both `HostView` and `PlayerView`, added in a follow-up commit (`fix: make phase-switch exhaustiveness actually enforced by tsc`).
7. **Task 12's commit did not use `git add -A`**, despite Step 9's code block. Every commit in this history stages explicit paths, so the untracked working note at the repo root stays untracked throughout.
8. **Kick was rebuilt; Task 8/9's version did nothing durable.** As originally specified, the server closed the kicked player's sockets and called it done — but `partysocket` auto-reconnects on its own, and the join gate only ever rejected *unknown* players outside the lobby, so a kicked player silently rejoined within about a second as a "new" player. The shipped version (commit `fix: make kick durable and return the kicked player to Landing`) adds a `kicked: PlayerId[]` list to `Room`, refuses any connection whose id is on it, and sends that player's device back to the landing screen instead of leaving it on a dead socket.
